...
class Mage_Core_Model_App {
	...
	private $INIT_FIRST = false;
	public $FPC_TOPCART=".block.topcart";
	public $FPC_TOPBAR=".block.topbar";
	
	public $FPC_TOPCART_DEFAULT=".block.topcart.default";
	public $FPC_TOPBAR_DEFAULT=".block.topbar.default";
	...
	public function run($params) {
		...
		Mage::log ( "get:$requestURI" );
		else
		Mage::log ( "post:$requestURI" );

		$options = isset ( $params ['options'] ) ? $params ['options'] : array ();
		$this->baseInit ( $options );

		$frontend = $this->getCookie()->get('frontend');
		//$customer = $this->getCookie()->get('customer');

		//if($frontend)
		//$cachekey = $frontend . $requestURI ;
		//else
		$cachekey = $requestURI;
		...
		if ($this->getRequest ()->isGet ()) {
			if ($_SERVER ['FULL_PAGE_CACHING']) {
				//后台不作全页缓存

				if(! $this->isNoCacheURL($requestURI))
				{
					$content = $this->loadCache ( $cachekey );
					if ($content)
					Mage::log ( "from full page caches get - $cachekey" );
				}
			}
		}
		...
		if ($content && !$frontend) {
			...
			Mage::log('cache and no frontend');

		}
		else if($content && $frontend)
		{
			$cartCache = $this->loadCache($frontend . $this->FPC_TOPCART);
			if( $cartCache  && ($cartCache=='update'))
			{
				Mage::log('generate cart cache');
				
				if(! $this->INIT_FIRST)
				$this->initFirst();
				
				$cartCache = $this->generateTopCartBlock();
				$this->saveCache($cartCache,$frontend . $this->FPC_TOPCART,array(),3600);
			}
			else
			Mage::log('get cart cache');

			if($cartCache)
			{
			$content = $this->replaceTopCart($content, $cartCache)  ;
			//if($customer){
			...
			$topBarCache = $this->loadCache($frontend . $this->FPC_TOPBAR);
//			if(!$topBarCache)
//			{
//				if(! $this->INIT_FIRST)
//				$this->initFirst();
...
//			}
			...
			$content = $this->replaceTopBar($content,$topBarCache);
			...
			$front = $this->getFrontController ()->dispatch ();
			if ($_SERVER ['FULL_PAGE_CACHING']) {
				$content = $front->getResponse ()->__toString () ;
				
				$topCartBlock=$this->getTopCart($content);
				$topBarBlock=$this->getTopBar($content);
				$topCartDefault=false;
				$topBarDefault=false;
				
				
				if(preg_match('/\/customer\/account\/login\//',$topBarBlock))
				{
					$topBarDefault=true;
					$this->saveCache($topBarBlock, $this->FPC_TOPBAR_DEFAULT,array(),36000);
				}
				
				if(preg_match('/<strong>0 /',$topCartBlock))
				{
					$topCartDefault=true;
					$this->saveCache($topCartBlock, $this->FPC_TOPCART_DEFAULT,array(),36000);
				}
				
				if($topBarDefault && $topCartDefault )
					$this->saveCache ( $content, $cachekey, array (), 36000 );
				else {
					if(! $topCartDefault)
						$content = $this->replaceTopCart($content, $this->loadCache($this->FPC_TOPCART_DEFAULT));
					if(! $topBarDefault)
						$content = $this->replaceTopBar($content, $this->loadCache($this->FPC_TOPBAR_DEFAULT));
				...
				if($frontend)
				{
					$this->saveCache($topCartBlock,$frontend . $this->FPC_TOPCART,array(),3600);
					$this->saveCache($topBarBlock,$frontend . $this->FPC_TOPBAR,array(),3600);
				}
			}
	...
	protected function generateTopCartBlock()
	{
		Mage::getSingleton('core/session', array('name' => 'frontend'));
				$cartDiv = $this->getLayout()->getBlockSingleton('checkout/cart_sidebar')->setTemplate("checkout/cart/topcart.phtml")->toHtml() ;
				return $this->getTopCart($cartDiv);
	}
	
	protected function isNoCacheURL($requestURI)
	{
		$noCachePageList = array(
					'/^\/admin\//',
					'/^\/cachecontrol\//',
					'/^\/downloader\//',
				   '/^\/checkout\/onepage\//',
					...
					$noCache = false;
					foreach ($noCachePageList as $url)
					{
	...
	protected function getTopBar($html)
	{
		preg_match('/<div class="top-bar"(.*)top-bar -->/s', $html, $matches);
		return $matches[0];
	}

	protected function replaceTopBar($html,$topBar)
	{
		if($topBar)
			return preg_replace('/<div class="top-bar"(.*)top-bar -->/s', $topBar , $html);
		else
			return $html;
	}

	protected function getTopCart($html){
		preg_match('/<div class="mini-cart"(.*)cart-sidebar \/\/ -->/s', $html, $matches);
		return $matches[0];
	}

	protected function replaceTopCart($html,$topCart) {
		if($topCart)
			return preg_replace('/<div class="mini-cart"(.*)\.cart-sidebar \/\/ -->/s', $topCart , $html);
		else
			return $html;
	}
	...
	protected function _checkCookieStore($type) {
		...
		if ($store && isset ( $this->_stores [$store] ) && $this->_stores [$store]->getId () && $this->_stores [$store]->getIsActive ()) {
			if ($type == 'website' && $this->_stores [$store]->getWebsiteId () == $this->_stores [$this->_currentStore]->getWebsiteId ()) {
				$this->_currentStore = $store;
			}
			if ($type == 'group' && $this->_stores [$store]->getGroupId () == $this->_stores [$this->_currentStore]->getGroupId ()) {
				$this->_currentStore = $store;
			}
			if ($type == 'store') {
				$this->_currentStore = $store;
	...
	protected function _initStores() {
		...
		$websiteCollection = Mage::getModel ( 'core/website' )->getCollection ()->initCache ( $this->getCache (), 'app', array (Mage_Core_Model_Website::CACHE_TAG ) )->setLoadDefault ( true );
		$groupCollection = Mage::getModel ( 'core/store_group' )->getCollection ()->initCache ( $this->getCache (), 'app', array (Mage_Core_Model_Store_Group::CACHE_TAG ) )->setLoadDefault ( true );
		$storeCollection = Mage::getModel ( 'core/store' )->getCollection ()->initCache ( $this->getCache (), 'app', array (Mage_Core_Model_Store::CACHE_TAG ) )->setLoadDefault ( true );
	...
	protected function _getDefaultStore() {
		if (empty ( $this->_store )) {
			$this->_store = Mage::getModel ( 'core/store' )->setId ( self::DISTRO_STORE_ID )->setCode ( self::DISTRO_STORE_CODE );
		}
		return $this->_store;
	}
