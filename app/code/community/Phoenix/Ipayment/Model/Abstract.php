...
abstract class Phoenix_Ipayment_Model_Abstract extends Mage_Payment_Model_Method_Abstract
{
	...
	public function authorize(Varien_Object $payment, $amount)
	{
		...
		//$payment->getOrder()->addStatusToHistory($this->getConfigData('order_status'), Mage::helper('phoenix_ipayment')->__('Payment has been preauthorized.'));
	...
	public function capture(Varien_Object $payment, $amount)
	{
        ...
        $options = array('transactionData' => array(
            ...
            //   -> use "capture" instead
        if ($payment->getLastTransId() && $order->getTotalInvoiced() == 0) {
        ...
        //$payment->getOrder()->addStatusToHistory($this->getConfigData('order_status'), Mage::helper('phoenix_ipayment')->__('Payment has been authorized and captured.'));
	...
	public function cancel(Varien_Object $payment, $amount = null)
	{
		...
		if ($res === false) {
			try {
				...
				$payment->getOrder()->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('phoenix_ipayment')->__('Payment has been refunded.'));
			}
			catch (Mage_Core_Exception $e) {
					// if the payment can't be refunded add an error message and continue
				Mage::getSingleton('adminhtml/session')->addError(Mage::helper('phoenix_ipayment')->__('Ipayment error: %s', $e->getMessage()));
				$payment->getOrder()->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('phoenix_ipayment')->__('Failed to refund payment.'));
			}
		} else {
			$payment->getOrder()->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('phoenix_ipayment')->__('Payment has been reversed.'));
		}
	...
	protected function getService()
	{
		if (!is_object($this->_service)) {
			try {
				...
				Mage::throwException(Mage::helper('phoenix_ipayment')->__('Can not connect payment service. Please try again later.'));
			}
		}
		...
		if (!is_array($this->_paymentData)) {
	        ...
	        $this->_paymentData =	array(
				...
				'addressData'	=>	array(
                    ...
                    'addrStreet'   =>   Mage::helper('core')->removeAccents($this->_normalizeStreet($billing->getStreet())),
                    'addrCity'     =>   Mage::helper('core')->removeAccents($billing->getCity()),
	...
	protected function _processRequest($request, $options, Varien_Object $payment)
	{
		...
		try {
            ...
            Mage::throwException(Mage::helper('phoenix_ipayment')->__('Can not prepare a request for the payment service. Please check your data.'));
        }
