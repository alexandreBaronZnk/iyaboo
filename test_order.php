    ...
    public function addressesPostAction() {
        ...
        try {
            ...
            $this->_getCheckoutSession()->addException(
                    $e, Mage::helper('checkout')->__('Data saving problem')
            );
            $this->_redirect('*/*/addresses');
        }
    }
...
//anderson
    public function addProductToVirtualOrder($quote, $orderid) {
        ...
        foreach ($items as $itemId => $item) {
            ...
            Mage::log('add product ' . $product->getName() . ' ' . $qty);
...
//anderson
    public function createVirtualOrder() {
        ...
        $quote->setCustomerEmail('multishipping@example.com');

        $product = Mage::getModel('catalog/product')->load(162);
        $buyInfo = array(
            'qty' => 2
        );
        $quote->addProduct($product, new Varien_Object($buyInfo));
...
//            $this->addProductToVirtualOrder($quote, $orderid);
            $order2 = Mage::getModel('sales/order')->loadByIncrementID($orderid);
            $items = $order2->getItems();
            Mage::log('add order product ' . $orderid);
        ...
        Mage::log('mutlshipping: ' . implode(',', $this->_getCheckout()->getOrderIds()));
...
//$order->setIsVirtual(1);
        $order->setCustomerNote(implode(',', $this->_getCheckout()->getOrderIds()))->save();
        Mage::log($order->getPayment()->getMethodInstance()->getCode());
...
//add by anderson
//$this->_redirect('*/onepage/saveOrder',array('multishipping'=>'true'));
//if ($data = $this->getRequest()->getPost('payment', false)) {
//    Mage::log($data);
//    $this->getOnepage()->getQuote()->getPayment()->importData($data);
//    //$this->getOnepage()->getQuote()->getPayment()->importData(array("method"=>"paypal_standard"));
//}
//Mage::log($this->getOnepage());
//$this->getOnepage()->saveOrder();
            $order = $this->createVirtualOrder();
            $this->_getCheckoutSession()->setLastRealOrderId($order->getIncrementId());
            $this->_getCheckoutSession()->setLastOrderId($order->getIncrementId());
            Mage::app()->getResponse()->setRedirect($paymentInstance->getOrderPlaceRedirectUrl());
//$redirectUrl = $this->_getCheckout()->getRedirectUrl();
//end by anderson
